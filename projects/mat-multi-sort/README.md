...
The `TableData` a useful class, which handles a lot of work for your app, such as page events (`next`, `previous`, `first`, `last`, `sizeChange`) and sorting events. Next to that, it keeps the current state of the table, again sorting and pagination.
...
| nextObservable     | An `Observable` that fires, when the user clicks the `nextPage` button                                                                                |                                  |                                              |
| previousObservable | An `Observable` that fires, when the user clicks the `previousPage` button                                                                            |                                  |                                              |
| firstObservable    | An `Observable` that fires, when the user clicks the `firstPage` button                                                                               |                                  |                                              |
| lastObservable     | An `Observable` that fires, when the user clicks the `lastPage` button                                                                                |                                  |                                              |
